...
<div class="container">
    <table class="table text-center table-bordered table-blue">
        ...
        <c:forEach items="${userOrders}" var="order">
            <tr>
                <td>${order.getId()}</td>
                <td>${order.getApplianceEntity().getName()}</td>
                <td>${order.getApplianceEntity().getManufacturerEntity()}</td>
                <td>${order.getApplianceEntity().getModel()}</td>
                <td>${order.getApplianceEntity().getDisrepair()}</td>
    ...
    <ul class="pagination">
        <c:if test="${currentPage != 1}">
            <li class="page-item"><a class="page-link"
                                     href="userOrders?recordsPerPage=${recordsPerPage}&currentPage=${currentPage-1}">Previous</a>
            </li>
        </c:if>
        ...
        <c:forEach begin="1" end="${noOfPages}" var="i">
            <c:choose>
                ...
                <c:otherwise>
                    <li class="page-item"><a class="page-link"
                                             href="userOrders?recordsPerPage=${recordsPerPage}&currentPage=${i}">${i}</a>
                    </li>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        ...
        <c:if test="${currentPage lt noOfPages}">
            <li class="page-item"><a class="page-link"
                                     href="userOrders?recordsPerPage=${recordsPerPage}&currentPage=${currentPage+1}">Next</a>
            </li>
        </c:if>
    </ul>
</nav>
