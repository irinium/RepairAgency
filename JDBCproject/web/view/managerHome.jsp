    ...
    <nav class="navbar navbar-expand-lg navbar-light">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="${pageContext.request.contextPath}/orderList" <fmt:message key="manager.toolbar.allOrders"
                                                                      var="orders"/>> ${orders}</a>
                </li>
                <li class="nav-item">
            ...
            <div class="col-md-6">
                <form class="appl-form" method="post" action="${pageContext.request.contextPath}/command">
                    ...
                    <div class="form-group">
                        <input class="form-control" name="master_email" type="text" <fmt:message key="placeholder.enter.email" var="em"/> placeholder="${em}"/>
                        <input class="form-control" name="password" type="password" pattern="^\S{8,}$" onchange="this.setCustomValidity(this.validity.patternMismatch ? 'Must have at least 8 characters' : ''); if(this.checkValidity()) form.password_two.pattern = this.value;" <fmt:message key="placeholder.enter.password" var="pas"/> placeholder="${pas}" required/>
                        <input class="form-control" name="password_two" type="password" pattern="^\S{8,}$" onchange="this.setCustomValidity(this.validity.patternMismatch ? 'Please enter the same Password as above' : '');" <fmt:message key="placeholder.enter.password2" var="pas2"/> placeholder="${pas2}" required/>
                        <input class="form-control" name="master_name" type="text" <fmt:message key="placeholder.enter.name" var="name"/> placeholder="${name}"/>
                        <input class="form-control" name="surname" type="text" <fmt:message key="placeholder.enter.surname" var="surname"/> placeholder="${surname}"/>
                        <input class="form-control" name="phone" type="text" <fmt:message key="placeholder.enter.phone" var="phone"/> placeholder="${phone}"/>
                    </div>
                    <button class="btn btn-primary" <fmt:message key="button.registration" var="register"/>>${register}</button>
                </form>
            </div>
