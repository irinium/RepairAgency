    ...
    <table class="table text-center table-bordered table-blue">
        ...
        <c:forEach items="${orders}" var="order">
            <tr>
                ...
                <td>
                    <label for="state"></label><select id="state" name="state" onchange="submit()"/>
                    <option value="false" ${state == 'false' ? 'selected' : ''} <fmt:message key="select.state1" var="notacc"/>>${notacc}</option>
                    <option value="true" ${state == 'true' ? 'selected' : ''} <fmt:message key="select.state2" var="accep"/>>${accep}</option>
                </td>
                <td>
                ...
                <td>
                    <button class="btn btn-primary" <fmt:message key="button.edit"
